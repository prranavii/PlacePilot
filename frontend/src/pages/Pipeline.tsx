...
export const Pipeline: React.FC = () => {
  ...
  const getStageBorderColor = (stage: string) => {
    switch (stage) {
      case 'Wishlist': return 'border-t-4 border-t-life-cocoa/30';
      case 'Applied': return 'border-t-4 border-t-life-vermilion';
      ...
      default: return 'border-t-4 border-t-life-cocoa/20';
    }
  };

  const getStageHeaderGlow = (stage: string) => {
    switch (stage) {
      case 'Wishlist': return 'text-life-cocoa/70 bg-life-cocoa/5 border-life-cocoa/15';
      case 'Applied': return 'text-life-vermilion bg-life-vermilion/5 border-life-vermilion/20';
      ...
      default: return 'text-life-cocoa/60 bg-life-cocoa/5 border-life-cocoa/10';
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex items-center gap-2 text-xs font-semibold bg-white border border-life-cocoa/10 px-4 py-2 rounded-xl text-life-cocoa shadow-sm dark:bg-zinc-900/60 dark:border-white/5 dark:text-zinc-300">
          <Activity className="w-4 h-4 text-life-vermilion animate-pulse" />
        ...
        {STAGES.map((stage) => {
          ...
          return (
            <div
              ...
              className="bg-white/70 border border-life-cocoa/5 rounded-2xl p-4 min-h-[70vh] flex flex-col gap-4 shadow-md backdrop-blur-md dark:bg-zinc-900/35 dark:border-white/5"
            >
              {/* Column Header */}
              <div className="flex items-center justify-between pb-3 border-b border-life-cocoa/5 mb-1 shrink-0">
                <span className={`text-xs font-bold uppercase tracking-wider px-2.5 py-1 rounded-full border ${getStageHeaderGlow(stage)}`}>
                  {stage}
                </span>
                <span className="text-xs bg-life-sand border border-life-cocoa/10 text-life-cocoa/70 font-bold px-2 py-0.5 rounded-lg shrink-0 dark:bg-zinc-900 dark:border-white/5 dark:text-zinc-400">
                  {stageApps.length}
                </span>
              </div>
                  ...
                  {stageApps.length > 0 ? (
                    stageApps.map((app) => (
                      ...
                      >
                        ...
                        <p className="text-xs text-life-cocoa/60 font-medium truncate mt-1 dark:text-zinc-400">
                          {app.role}
                        </p>

                        <div className="mt-4 space-y-2 border-t border-life-cocoa/5 pt-3.5 dark:border-white/5">
                          {app.location && (
                        ...
                        {/* Card Footer tags */}
                        <div className="mt-4 flex items-center justify-between border-t border-life-cocoa/5 pt-3.5 dark:border-white/5">
                          <span className="text-[9px] text-life-cocoa/50 uppercase tracking-widest font-bold flex items-center gap-1.5 dark:text-zinc-400">
                            <span className={`w-1.5 h-1.5 rounded-full ${getPriorityDot(app.priority)}`} />
                            {app.priority}
                          </span>
                          {app.personal_readiness < 65 ? (
                            <span className="text-[9px] bg-life-vermilion/10 text-life-vermilion px-2 py-0.5 rounded font-bold flex items-center gap-0.5 animate-pulse">
                              <Sparkles className="w-3 h-3 fill-life-vermilion/10" /> Boost
                            </span>
                          ) : (
                            <span className="text-[9px] text-life-vermilion font-bold bg-life-vermilion/10 px-2 py-0.5 rounded flex items-center gap-0.5">
                    ...
                    <div className="border border-dashed border-life-cocoa/10 rounded-2xl py-12 text-center px-4 dark:border-white/5">
                      <span className="text-[10px] text-life-cocoa/40 font-bold uppercase tracking-wider block dark:text-zinc-500">Column Empty</span>
                      <span className="text-[9px] text-life-cocoa/20 block mt-1 dark:text-zinc-600">Drag applications here</span>
                    </div>
                  )}
                </AnimatePresence>
              </div>
