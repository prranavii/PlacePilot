...
export const Pipeline: React.FC = () => {
  ...
  const getStageHeaderGlow = (stage: string) => {
    switch (stage) {
      case 'Wishlist': return 'text-cocoa/80 bg-cocoa/5 border-cocoa/10';
      case 'Applied': return 'text-vermilion bg-vermilion/5 border-vermilion/20';
      ...
      default: return 'text-cocoa/60 bg-cocoa/5 border-cocoa/10';
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex items-center gap-2.5 text-[9px] font-bold bg-white border border-cocoa/10 px-4 py-2.5 rounded-full text-cocoa shadow-sm uppercase tracking-wider">
          <Activity className="w-4 h-4 text-vermilion animate-pulse" />
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center justify-between pb-3 border-b border-cocoa/10 mb-1 shrink-0">
                ...
                <span className="text-xs bg-sand border border-cocoa/10 text-cocoa/80 font-bold px-2 py-0.5 rounded-lg shrink-0 dark:bg-sand dark:border-cocoa/10 dark:text-cocoa/60">
                  {stageApps.length}
                </span>
              </div>
                  ...
                  {stageApps.length > 0 ? (
                    stageApps.map((app) => (
                      <motion.div
                        ...
                        className={`bento-panel cursor-grab p-4 relative overflow-hidden ${getStageBorderColor(stage)} ${
                          updatingId === app.id ? 'opacity-40 animate-pulse' : ''
                        }`}
                      >
                        <h4 className="font-bold text-cocoa text-sm tracking-tight truncate dark:text-zinc-200">
                          {app.company_name}
                        </h4>
                        <p className="text-xs text-cocoa/60 font-medium truncate mt-1 dark:text-cocoa/60">
                          {app.role}
                        </p>

                        <div className="mt-4 space-y-2 border-t border-cocoa/10 pt-3.5 dark:border-cocoa/10">
                          {app.location && (
                        ...
                        {/* Card Footer tags */}
                        <div className="mt-4 flex items-center justify-between border-t border-cocoa/10 pt-3.5 dark:border-cocoa/10">
                          <span className="text-[9px] text-cocoa/50 uppercase tracking-widest font-bold flex items-center gap-1.5 dark:text-cocoa/60">
                            <span className={`w-1.5 h-1.5 rounded-full ${getPriorityDot(app.priority)}`} />
                            {app.priority}
                          </span>
                          {app.personal_readiness < 65 ? (
                            <span className="text-[9px] bg-vermilion/10 text-vermilion px-2 py-0.5 rounded font-bold flex items-center gap-0.5 animate-pulse">
                              <Sparkles className="w-3 h-3 fill-vermilion/10" /> Boost
                            </span>
                          ) : (
                            <span className="text-[9px] text-vermilion font-bold bg-vermilion/10 px-2 py-0.5 rounded flex items-center gap-0.5">
