...
export const ResumeMatcher: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute -top-12 -left-12 w-96 h-96 bg-vermilion/5 rounded-full blur-3xl pointer-events-none animate-blob"></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        ...
        <div className="bg-white border border-cocoa/10 p-6 rounded-[1.5rem] shadow-md backdrop-blur-md space-y-5">
          <form onSubmit={handleMatch} className="space-y-5">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="block text-[9px] font-bold text-cocoa/50 uppercase tracking-wider dark:text-zinc-455">
                  Resume Content *
                </label>
                <div className="relative">
                  <label className="cursor-pointer text-[10px] font-bold text-vermilion hover:text-vermilion/85 flex items-center gap-1.5 transition-all">
                    {parsing ? (
                      <>
                        <span className="h-3.5 w-3.5 border-2 border-vermilion border-t-transparent rounded-full animate-spin" />
          ...
          <AnimatePresence mode="wait">
            {result ? (
              <motion.div 
                ...
                className="bg-white/70 border border-cocoa/10 p-6 rounded-[1.5rem] shadow-md backdrop-blur-md space-y-6 relative overflow-hidden dark:bg-sand/35 dark:border-cocoa/10"
              >
                
                {/* Match Score Gauge */}
                <div className="flex flex-col items-center pb-5 border-b border-cocoa/10">
                  <span className="text-[9px] font-bold uppercase tracking-widest text-cocoa/40">
                    Calculated Score
                  </span>
                  <div className="text-3xl font-bold text-vermilion mt-2 font-serif">
                    {result.match_percentage}%
                  </div>
                  <div className="text-xs font-semibold text-cocoa/70 tracking-tight mt-1 dark:text-cocoa/80">
                    Resume-Role Alignment
                  </div>
                  <div className="w-full bg-sand border border-cocoa/10 h-3 rounded-full mt-4 overflow-hidden p-0.5 dark:bg-cocoa dark:border-cocoa/10">
                    <motion.div 
                      initial={{ width: 0 }}
                      animate={{ width: `${result.match_percentage}%` }}
                      transition={{ duration: 0.8, ease: "easeOut" }}
                      className="bg-vermilion h-full rounded-full" 
                    />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <span className="text-[9px] text-cocoa/50 block font-bold uppercase tracking-wider dark:text-cocoa/40">
                    Match Explanation
                  </span>
                  <p className="text-xs text-cocoa/70 leading-relaxed bg-sand p-4 rounded-[1.2rem] border border-cocoa/10 dark:bg-cocoa/20 dark:text-cocoa/60 dark:border-cocoa/10">
                    {result.explanation}
                  </p>
                </div>
                ...
                {result.matched_skills.length > 0 && (
                  <div className="space-y-2.5">
                    <span className="text-[9px] text-cocoa/50 block font-bold uppercase tracking-wider flex items-center gap-1.5 dark:text-cocoa/40">
                ...
                {result.missing_skills.length > 0 && (
                  <div className="space-y-2.5">
                    <span className="text-[9px] text-cocoa/50 block font-bold uppercase tracking-wider flex items-center gap-1.5 dark:text-cocoa/40">
                      <AlertTriangle className="w-3.5 h-3.5 text-vermilion" /> Missing Skills / Gaps
                    </span>
                    <div className="flex flex-wrap gap-2">
                      {result.missing_skills.map((s: string, idx: number) => (
                        <span key={idx} className="text-[10px] font-bold bg-vermilion/10 text-vermilion px-3 py-1 rounded-full border border-vermilion/20">
                          {s}
                        </span>
                      ))}
                    </div>
                ...
                {result.keyword_gaps.length > 0 && (
                  <div className="space-y-2.5">
                    <span className="text-[9px] text-cocoa/50 block font-bold uppercase tracking-wider flex items-center gap-1.5 dark:text-cocoa/40">
                      <Search className="w-3.5 h-3.5 text-vermilion" /> Keyword Gaps
                    </span>
                    <div className="flex flex-wrap gap-2">
                ...
                {result.likely_interview_areas.length > 0 && (
                  <div className="space-y-3">
                    <span className="text-[9px] text-cocoa/50 block font-bold uppercase tracking-wider flex items-center gap-1.5 dark:text-cocoa/40">
                      <Lightbulb className="w-3.5 h-3.5 text-vermilion" /> Likely Interview Topics
                    </span>
                    <ul className="text-xs text-cocoa/85 list-disc list-inside space-y-2 bg-sand p-4 rounded-[1.2rem] border border-cocoa/10 leading-relaxed dark:bg-cocoa/20 dark:text-cocoa/60 dark:border-cocoa/10">
                      {result.likely_interview_areas.map((t: string, idx: number) => (
                        <li key={idx} className="marker:text-vermilion">{t}</li>
                      ))}
                    </ul>
                  </div>
                )}
              ...
              <div className="bg-white border border-cocoa/10 text-center py-20 rounded-[1.5rem] flex flex-col items-center justify-center p-8 shadow-lg">
                ...
                <h4 className="font-bold text-cocoa text-sm uppercase tracking-wider font-serif">No analysis generated</h4>
